...
function LoginForm() {
  ...
  const handleDigiLockerLogin = async () => {
    const res = await fetch("/api/auth/digilocker?action=authorize&state=login");
  ...
  return (
    <Card className="animate-card-in">
      <CardHeader className="text-center">
        ...
        <CardDescription>Sign in to your Niyomi account</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-2">
          <Button variant="outline" onClick={handleGoogleLogin} className="text-xs">
          ...
          <Button variant="outline" onClick={handleDigiLockerLogin} className="text-xs">
        ...
        <form onSubmit={handleEmailLogin} className="space-y-3">
          ...
          <div className="relative">
            ...
            <Input type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} className="pl-8 pr-8 h-9 text-xs" required />
          ...
          <div className="flex items-center justify-end">
            <Link href="/auth/forgot-password" className="text-[11px] text-muted-foreground hover:text-foreground">Forgot password?</Link>
          </div>
